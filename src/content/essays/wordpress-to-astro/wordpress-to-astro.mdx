...
newsletterIntro: |-
  Liking a site is not the same as loving it. Why I left the WordPress Site Editor for Astro and Markdown, what it cost, and what the site runs on now.
categories:
  - Web
tags:
  - Blogging
...
- Ordinary prose stays portable Markdown, while explicit MDX modules handle figures, galleries, summaries, callouts, and attributed quotations.
...
The checklist said go. The instinct said the backend was dead weight, and I have [written before about trusting that instinct](/do-you-trust-your-instincts-making-smart-wordpress-choices/) on WordPress decisions.
...
None of that makes it clever. It’s similarity, not comprehension, and it has [the same limits everything else in this family has](/limits-of-machine-learning/). It puts two essays together because their meaning points the same way, which is not the same as understanding either one. When it is wrong, it is confidently wrong.
...
The thing I learned isn’t that Astro is better than WordPress. It isn’t. It is the same lesson [the slow way I learned to build for the web](/what-is-a-web-developer/) keeps teaching me. They’re answers to different questions, and I was asking the wrong one for years without noticing.
