...
seoDescription: Create WooCommerce add-to-cart links for simple, variable, and grouped products, set quantities and redirects, then use them in buttons or emails.
date: 2015-09-18
updated: 2025-11-25
sticky: false
cornerstone: false
...
newsletterIntro: |-
  Learn how to create custom Add to Cart links with WooCommerce for various product types, enhancing the shopping experience.
categories:
  - Web
tags:
  - Performance
...
import productId from "./product-id.jpg";
import variationId from "./variation-id.jpg";
import addLinkToText from "./add-link-to-text.jpg";
import addButtonWithLink from "./add-button-with-link.jpg";
...
<Figure src={productId} alt={"The products list in Products > All Products, highlighting the product ID of a simple product."} />
...
With new WooCommerce versions, this process is simpler. It’s like with simple products, but use the variation ID found under Variations on the Edit Product page. If you are not sure whether you need a variation at all, [the difference between attributes and variations](/woocommerce-attributes-vs-variations/) decides it.

<Figure src={variationId} alt={"The Variations tab of a variable product highlighting the variation ID."} />
...
<Figure src={addLinkToText} alt={"The WordPress content editor showing how to add the add to cart URL to a text link in"} />

- Or using a [Button block](https://wordpress.org/documentation/article/buttons-block/):

<Figure src={addButtonWithLink} alt={"The WordPress content editor showing how to add the add to cart URL to the Button block."} />
