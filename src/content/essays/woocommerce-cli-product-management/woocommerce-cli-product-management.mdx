...
seoDescription: Manage WooCommerce products from the terminal, including variable products, stock, prices, and bulk changes, with clear and repeatable CLI commands.
date: 2016-03-10
updated: 2016-03-10
sticky: false
cornerstone: false
...
newsletterIntro: |-
  Manage WooCommerce products from the terminal, including variable products and bulk changes, with clear, scriptable CLI commands.
categories:
  - Web
tags:
  - Performance
...
Coupons are their own post type and the CLI treats them the same way, which is one route to [clearing out expired ones](/delete-expired-coupons-automatically-woocommerce/).
...
Variable products combine attributes (like Color or Size) with multiple variations, each with its own price and settings. Understanding the difference between attributes and variations can take a moment, so [I wrote a detailed essay about it](/woocommerce-attributes-vs-variations/).
