...
import rollLog from "./roll-log.jpg";
...
- A tool can show every die and still leave you trusting that those were the dice it rolled.
...
- By default, it uses `crypto.getRandomValues` rather than `Math.random`, removes modulo bias by rejection, draws once per die, and never nudges a result.
- I designed much of the library around what it refuses to do: no division, no game rules, and no line breaks inside a formula.
...
And when we packed up, I could not tell my players otherwise. I could say, "You just got unlucky with the rolls," only if I knew it was true, and I did not. I had watched the same numbers they had. That was all either of us had.

That session was the last drop.

## Showing The Work Is Not The Same As Knowing

That fight was not the whole story. The app I used for rolling had been wearing on me for a while, in smaller ways that mostly had nothing to do with dice. Those belong in a post of their own. What that night added was distrust, and distrust was the one item on the list I could not fix by moving to a different app.

I had not found anything wrong with it. I had not looked.

I could have investigated the app or built my own. Building was more interesting, and with ADHD that is not a fair fight. [Novelty beats repetition](/tools-for-adhd-leadership/), and it wins before I have finished thinking it through.

Reading somebody else's random number generator felt repetitive. Writing my own was new. So I wrote my own and left the question of whether I needed to for much later.

The app did show me its dice: every roll broken out, every die listed, and the arithmetic on screen.

But I still had to trust that the dice on screen were the dice it had rolled.
...
It was its own library from the first commit rather than something I carved out later, and I published it on npm as [`opendice`](https://www.npmjs.com/package/opendice). Anybody can use it to roll dice in JavaScript or TypeScript, whether for D&D or something unrelated to games. Install it with:
...
Five, plus three, plus three. Eleven. I can show somebody the arithmetic instead of shrugging at a total.
...
| `4d6kh`        | Leave the number out to keep one die.                       |
| `2d20adv`      | Roll two d20 and keep the higher one. Advantage.            |
| `2d20dis`      | Roll two d20 and keep the lower one. Disadvantage.          |
| `2d6min3`      | Any die below 3 counts as 3. `max` sets a ceiling.          |
| `2d6totalmin8` | The two dice together count at least 8. `totalmax` caps it. |
| `1d6!`         | Exploding. Every top face rolls again.                      |
| `1d6!p`        | Penetrating. Extra rolls count for one less.                |
...
The number of dice is always the number rolled. `2d20adv` rolls two and keeps one; `4d20adv` rolls four and keeps one. One die leaves nothing for `adv` or `dis` to choose between, so `1d20adv` is refused.
...
### Exploding And Penetrating Dice
...
Add a `p` for penetrating dice. They explode the same way, but each roll after the first is worth one less. HackMaster uses this version.

```ts
roll('1d6!p') // rolled 6, then 4    → results [6, 3],    total 9
roll('1d6!p') // rolled 6, 6, then 2 → results [6, 5, 1], total 12
```

The deduction changes what the extra roll is worth, not the face that decides whether the chain continues. That second 6 is recorded as 5 and still triggers another roll.

### Floors And Ceilings

`min` gives every die a floor, while `max` gives it a ceiling.

```ts
roll('2d6min3') // rolled 1 and 5 → the dice count as 3 and 5
roll('2d6max5') // rolled 6 and 4 → the dice count as 5 and 4
```

The dice are not rerolled or hidden. The bound joins them in `results`, and `keptFlags` shows whether the die or the bound counted.

```ts
const bounded = roll('2d6min3') // rolled 1 and 5

bounded.dice[0].results   // [1, 3, 5, 3]
bounded.dice[0].kept      // [3, 5]
bounded.dice[0].keptFlags // [false, true, true, false]
```

Put `total` in front to apply the floor or ceiling after the dice are added together. A `2d6totalmin8` that rolls 3 and 2 is worth 8; `2d6totalmax8` never counts for more than 8.
...
That also makes OpenFray the same kind of tool as the one that annoyed me into writing it.
...
Keeping randomness in one place gave me one place to check it. If I had scattered `Math.random()` across the codebase, every call site could have done things a little differently. Behind a single `roll()`, the answer to "how does this app roll dice" is a file somebody can point at.

I made that choice from the start. OpenFray uses the package the same way anyone else would.

Publishing it as a package made me add a version, a README, a test suite, and an API that could not quietly change shape because it suited me that week. Those constraints help whoever comes along later, and they make the library easier to check.

### MIT, While OpenFray Is AGPL

OpenFray is licensed AGPL-3.0. The dice are MIT.

I chose MIT for the dice so people could reuse them without having much to weigh up. Someone building another tabletop tool, a game, or a teaching example should be able to take them without reading the license twice.

The app is a different matter and gets a different answer. I went through that choice at length in [a previous essay](/how-to-choose-a-software-license-for-your-next-project/). It looks like a legal maze from outside, and mostly comes down to what you want to happen next.

## Showing The Work

I built the result object so my players could reconstruct a total without having to trust me.
...
```ts
const r = roll('2d20adv')

r.dice[0].results        // [4, 17]       — both dice
r.dice[0].kept           // [17]          — the one that counted
r.dice[0].keptFlags      // [false, true] — one marker for each result
r.dice[0].advantageState // 'advantage'   — why one die counted
```

I keep the 4 because "taking the higher" only means anything if you can see what was not taken. A 17 on its own tells you the result. A 4 and a 17, with the 17 kept, shows you how the library chose it.
...
Showing both numbers lets my players check the −5 instead of taking it on trust.

<Figure src={rollLog} alt={"OpenFray's game log filtered to rolls. A disadvantage check shows both d20s, 8 and 3, resolving to 3. An advantage attack shows 1 and 18, resolving to 18. A strength check lists its modifiers as +1 and -6 rather than as a single -5."}>
  Both of the rules above, running in OpenFray. The die that lost is still printed next to the one that won, and the exhaustion penalty sits beside the bonus instead of being folded into it.
</Figure>
...
This does not make the dice fairer, but it does let me account for each roll. That was what I had been missing.
...
`parseFormula()` reads a formula and tells you what it means, without throwing any dice. I use it to validate something a person typed, so a bad formula fails where it was written rather than three screens later.
...
**`advantage`** does the same job as writing `adv` into the formula. The formula still needs at least two dice; the option decides which one to keep rather than quietly adding another. The formula version suits a fixed roll, while the option suits one your code decides at run time. If several things in your project would each want to set it, work out the net result yourself and pass one answer. The library has no rules of its own to apply.
...
The default random source and bias removal need no options. They apply to every roll unless you supply your own source through `rand`.
...
`opendice` uses `crypto.getRandomValues` instead. It is available in browsers and Node, and it costs nothing at the scale a dice roller works at.
...
The library throws away the draws that would cause it and draws again, so every face is exactly as likely as every other. The fix costs a few lines and a rejection every few billion rolls, so I included it.

Every die also gets its own draw. The platform fills those random words in batches, but several dice are never squeezed out of one number. That is why a die can have at most 4,294,967,296 sides. One word covers exactly that many faces, and the fairness rests on one for each die.
...
I leave bad streaks alone. Any code that smoothed one out would turn the log into a lie, and I wrote this so I could hand that log to somebody.
...
That option is useful for tests and dangerous anywhere a player can reach it. A result records what the dice showed, never where the numbers came from, so a rigged source looks like a fair one. If someone using your program can choose the source, they can choose the roll.

I exported `cryptoRandom()` so you can call the raw source directly, inspect it, wrap it, or test it. Nobody has to take my word for how the dice work. After that night, taking somebody's word was what I had run out of patience for.

## What It Won't Do

I spent more time deciding what the library would refuse than what it would do. Each refusal leaves the choice to the code using it.
...
**Exploding and penetrating dice will not combine with keep-highest.** `4d6kh3!` is rejected rather than guessed at. "Keep the best three" stops meaning anything once every die is an open-ended chain. There is a defensible reading of it. There are also two or three others, and picking one silently would be worse than saying no.

**Bounds will not combine with keep rules, advantage, disadvantage or explosions.** `4d6kh3min2` could mean applying the floor before choosing the best three, or after. The library refuses to pick one without telling you.
...
| 100 explosions or penetrations | A fair d6 reaching ten in a row is a one-in-sixty-million event.           |
...
Finally, **the library has no game rules.** It rolls dice and reports what happened. Whether a 20 is good, a label means fire damage, or a total passes or fails is up to the code using it.
...
Games have taught me more about running things than most of my working life has, which I have [written about before](/gaming-made-me-better-leader/). This one taught me that showing the work earns more trust than reassurance does.

Writing the dice took a few weekends, in the [bursts I have learned to expect](/adhd-planner/). Deciding what they would refuse to do took considerably longer, and that is the part I would keep.
