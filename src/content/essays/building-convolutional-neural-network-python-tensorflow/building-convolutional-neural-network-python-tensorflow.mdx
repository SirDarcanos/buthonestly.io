...
seoTitle: Convolutional Neural Network in Python with TensorFlow
seoDescription: Build a convolutional neural network in TensorFlow, from data preparation and layer design to training, evaluation, and Bayesian tuning.
date: 2025-10-30
updated: 2026-05-12
sticky: false
cornerstone: false
...
import mnistDatasetSample from "./mnist-dataset-sample.jpg";
import optimizerPath from "./optimizer-path.jpg";
import vgg16 from "./vgg-16.jpg";
import cnnAccuracyPlot from "./cnn-accuracy-plot.jpg";
import cnnOptimizedAccuracyPlot from "./cnn-optimized-accuracy-plot.jpeg";
import cnnModelPredictions from "./cnn-model-predictions.jpg";
import cnnConfusionMatrix from "./cnn-confusion-matrix.jpg";
...
If not, I’ve written a dedicated guide that walks through how to [set up a TensorFlow Docker Jupyter environment step by step](/set-up-tensorflow-docker-jupyter-notebook/).
...
<Figure src={mnistDatasetSample} alt={"MNIST dataset sample"} />
...
Right after that, the constructor checks your hardware. If there’s more than one [GPU](/what-is-a-gpu/), it prepares a mirrored strategy so the same model runs on all of them in parallel. If there isn’t, it quietly falls back to a single GPU or CPU. You don’t need to change any training code for either case.
...
<Figure src={optimizerPath} alt={"Illustration of the path followed by the gradient descent method to reach the minimum of a"}>
  Image by ResearchGate.net.4
</Figure>
...
<Figure src={vgg16} alt={"The standard VGG-16 neural network architecture."}>
  Image by ResearchGate.net.6
</Figure>
...
<Figure src={cnnAccuracyPlot} alt={"CNN baseline accuracy plot"} />
...
<Figure src={cnnOptimizedAccuracyPlot} alt={"CNN optimized accuracy plot"} />
...
<Figure src={cnnModelPredictions} alt={"CNN model predictions"} />
...
<Figure src={cnnConfusionMatrix} alt={"CNN confusion matrix"} />
