...
seoTitle: DistilRoBERTa Emotion Analysis on Steam Reviews
date: 2025-11-20
updated: 2026-05-13
sticky: false
cornerstone: false
...
import dbdReviewsNlpSentimentPlot from "./dbd-reviews-nlp-sentiment-plot.jpg";
import dbdSteamReviewsNlpNurseAnalysis from "./dbd-steam-reviews-nlp-nurse-analysis.jpg";
...
At some point I realized I could do something useful with that. Other than [learning more about leadership](/gaming-made-me-better-leader/) that is.
...
If you don’t have an environment setup already, I explained in another essay [how to set one up](/set-up-tensorflow-docker-jupyter-notebook/). Instead of using the image from that tutorial, follow the steps but download `quay.io/jupyter/pytorch-notebook:latest`.
...
Memory-wise, the whole thing sits around **25 MB**, so you can open it on a laptop without needing a [GPU](/what-is-a-gpu/) shrine in the corner of your room. As I am writing this, I am indeed running the code on a MacBook Pro with no external GPU.
...
<Figure src={dbdReviewsNlpSentimentPlot} alt={"Steam reviews sentiment analysis over time vs. game patches."} />
...
Under the hood, this is just a small Transformer model from the `transformers` library, running with a PyTorch backend. [Depending on your environment](/set-up-tensorflow-docker-jupyter-notebook/), you may need to install a couple of extra packages first: `pip install transformers`
...
km["year_month", "review_count", "share_positive"](/"year_month", "review_count", "share_positive"/).head(), km[
    ["year_month", "review_count", "share_positive"]
].tail()
```
...
<Figure src={dbdSteamReviewsNlpNurseAnalysis} alt={"Pretrained emotion classifier on Dead by Daylight Steam reviews."} />
