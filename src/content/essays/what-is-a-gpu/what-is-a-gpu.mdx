...
seoDescription: Learn what a GPU is, how it differs from a graphics card, and why parallel processing, VRAM, and memory bandwidth matter so much for AI.
date: 2026-08-04
updated:
sticky: false
cornerstone: false
...
import nvidiaGeForceRtx5090 from "./nvidia-geforce-rtx-5090.jpg";
import gpu from "./gpu.jpg";
import rtx5090Pcb from "./rtx-5090-pcb.jpg";
import pcbDiagram from "./nvidia-geforce-rtx-5090-pcb-diagram.jpg";
import gpuCores from "./nvidia-geforce-rtx-5090-gpu-cores.jpg";
import h100 from "./h100-ari.jpeg";
import appleM4 from "./apple-m4-abstract.jpg";
import ethereumMarket from "./ethereum-market.jpg";
...
<Figure src={nvidiaGeForceRtx5090} alt="NVIDIA GeForce RTX 5090 graphics card, full assembly with cooling shroud and fans.">
  Image by NVIDIA
</Figure>
...
Below: two images by _W1zzard_ on _techpowerup.com_ — the GB202 chip itself, and the PCB it sits on in an RTX 5090.

<Gallery columns={2}>
  <Figure src={gpu} alt="NVIDIA GB202 GPU die, the bare silicon chip without its surrounding components." />
  <Figure src={rtx5090Pcb} alt="NVIDIA GeForce RTX 5090 printed circuit board with the GB202 GPU at center, surrounded by VRAM modules and power delivery components." />
</Gallery>
...
<Figure src={pcbDiagram} alt="Annotated diagram of the NVIDIA RTX 5090 printed circuit board, with display outputs, power connector, VRAM modules, and the GB202 GPU labeled." />
...
<Callout type="information" title="Ray tracing, briefly explained">
...
<Figure src={gpuCores} alt="Block diagram of the NVIDIA RTX 5090 GPU showing the layout of CUDA cores, Tensor cores, and RT cores across the chip." />
...
<Figure src={h100} alt="NVIDIA H100 data-center GPU card with no display outputs and no fans, designed for server-rack installation.">
  Image by NVIDIA
</Figure>
...
<Figure src={appleM4} alt="An abstract image of an Apple M4 chip centered on a neon circuit board with glowing purple traces and green lights.">
  Photo by [BoliviaInteligente](https://unsplash.com/@boliviainteligente?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText) on [Unsplash](https://unsplash.com/?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText)
</Figure>

There's a third path I've used: Apple Silicon. The trick is unified memory. On a Mac with an M-series chip, the GPU and CPU share the same memory pool. There's no separate VRAM with its own ceiling. A Mac with enough system RAM can sometimes load models that won't fit on a desktop card with much faster compute. The trade-off is speed: a Mac will train slower than a dedicated GPU, and not every framework supports its Metal backend cleanly. But for inference and for fitting larger models locally, it's a real option. The [NLP analysis I ran on Steam reviews](/distilroberta-emotion-analysis-nlp-case-study/) is the kind of work where memory ceiling matters more than raw speed.
...
<Figure src={ethereumMarket} alt="Ethereum logo on a coin over a blurred cryptocurrency candlestick chart with a purple and teal background.">
  Photo by [Yiğit Ali Atasoy](https://unsplash.com/@yigitaliatasoy?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText) on [Unsplash](https://unsplash.com/?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText)
</Figure>
